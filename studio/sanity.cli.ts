...
export default defineCliConfig({
  ...
  typegen: {
    ...
    // by the Next app next door. Reads GROQ from web/src/sanity/queries/*.ts (and any
    // other .ts/.tsx under web/src that calls defineQuery) and writes a single
    // generated file into web/. A query file outside this glob produces no types and
    // no error, so keep it wide enough to cover wherever queries end up living.
    path: '../web/src/**/*.{ts,tsx}',
    schema: 'schema.json',
