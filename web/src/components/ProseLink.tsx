 ...
 * ## The blue
 *
 * This and the link renderer in `docs/DocsMarkdown` are the only consumers of `--color-link`,
 * and both sit inside the scope DESIGN.md draws for it. The spec is a strict black-and-white
 * duet with exactly one chromatic exception, scoped tightly: the blue is "used only inside
 * long-form article body copy, never on UI buttons or navigation". Here it lives on the
 * annotation that only appears inside prose; there, on links inside the Field Guide's body,
 * which is the same kind of copy in a different source format. The two share their classes
 * verbatim — change both or neither.
 *
