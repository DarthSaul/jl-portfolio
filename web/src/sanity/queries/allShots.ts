 ...
 * cheap.
 *
 * It lives beside the queries it sizes because it now has **two** readers that must agree: the page
 * asks for the first slice with it, and `app/api/photos` clamps its own slice to it so a public
 * endpoint cannot be asked for the whole set at once. Two copies of this number would mean a cap
 * that silently stopped matching the page size.
 *
 * 24 is untested against real volume — the dataset holds ~28 photographs and she is expected to
 * upload ~250. The figure to re-measure is not the document count but the first page's LQIP weight,
 * roughly a kilobyte per photo. See CLAUDE.md.
 */
